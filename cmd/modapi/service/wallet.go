...
func WalletCreate(request request.WalletCreateRequest, tenant model.Tenant) (string, error) {
	chainDef, err := bip.CheckValidChainCode(request.Chain)
	if err != nil {
		return "", errors.New("unsupport chain:" + request.Chain)
	}
	var kdf bip.KDFParams
	err = json.Unmarshal([]byte(tenant.KdfParams), &kdf)
	if err != nil {
		return "", errors.New("tenant kdf params error:" + err.Error())
	}
	enc := bip.EncMaster{
	...
	if err := tx.Where("tenant_id = ? and chain = ?", request.TenantID, request.Chain).First(&tenantChain).Error; err != nil {
		...
		if err != nil {
			return "", errors.New("generate derivation chain error:" + err.Error())
		}
		...
		if err := tx.Save(&tenantChain).Error; err != nil {
			return "", errors.New("save tenant chain error:" + err.Error())
		}
	}
	...
	if err := tx.Save(&tenantAddress).Error; err != nil {
		return "", errors.New("save tenant address error:" + err.Error())
	}

	if err := tx.Commit().Error; err != nil {
		return "", errors.New("commit transaction error:" + err.Error())
	}
	committed = true
